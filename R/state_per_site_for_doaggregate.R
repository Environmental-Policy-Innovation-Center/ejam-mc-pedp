...
state_per_site_for_doaggregate <- function(s2b, s2st) {
  ...
  # Use ST or fips if already have it ####
  # but normally it is not yet here, if input was just something like testpoints_10  ***
  suppressWarnings({ suppressMessages({
    sites2states <- state_from_sitetable(s2st, ignorelatlon = TRUE) # 1st check only ST and FIPS, not latlon or polygon yet.
  })})
  setDT(sites2states)
  ...
  #
  # FOR SINGLE-STATE SITES, use bgid ####
  # (and it returns NA for multistate ones)
  ...
  if (!all(idst$ejam_uniq_id %in% id_gotten)) {
    # FOR MULTISTATE SITES ONLY, use latlon of site not blocks ####
    ...
    suppressWarnings({ suppressMessages({
      idst <- state_from_sitetable(s2st[!(ejam_uniq_id %in% id_gotten), ], ignorelatlon = FALSE)   # now use state_from_latlon() - SLOW
      setDT(idst)
      setkey(idst, ejam_uniq_id)
