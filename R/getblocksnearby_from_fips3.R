...
#' @keywords internal
#'
getblocksnearby_from_fips3 <- function(fips, in_shiny = FALSE, need_blockwt = TRUE) {
  ...
  if (NROW(fips_blockpoints) == 0) {
    if (in_shiny) {shiny::validate('No blockgroups found for these FIP codes.')} else {stop('No blockgroups found for these FIP codes.')}
  } else {
