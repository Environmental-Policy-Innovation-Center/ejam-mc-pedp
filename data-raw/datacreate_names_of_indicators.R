...
datacreate_names_of_indicators <- function(redohelp = FALSE) {
  ...
  ################################################################ #
  #### ***
  # assign metadata to all those lists???  assigning attributes to a vector like names_d makes it no longer a vector per is.vector() and is messy to print and not so useful.
  # for (i in seq_along(vlists)) {
  #   assign(vlists[i], metadata_add(get(vlists[i])))
  # }

  ### see which dataset objects are atomic but no longer vector since attributes got assigned
  x = pkg_data('EJAM')$Item
  # for (ob in x) {
  #   vec = is.vector(try(get(ob)))
  #   yeah = is.atomic(try(get(ob)))
  #   if (yeah) {cat("atomic? ",  yeah)
  #     cat("  vector? ", vec)
  #     cat(" - " , ob, "\n") }
  # }
  atomic_nonvectors <- data.frame(dname = x, yes = NA)
  atomic_nonvectors$yes <- sapply(x,
                                  function(z) {is.atomic(try(get(z))) & !is.vector(try(get(z)))}
                              )
  atomic_nonvectors[atomic_nonvectors$yes,]
  names_datasets = x[grepl("^names_", x)]
  # names_datasets

  ## to undo that
  make_vector_and_use_data = function(dataset_names, checking=TRUE) {
    for (ob in dataset_names) {
      codetosource <- paste0( ob , " <- as.vector( ", ob,  ");",
                              paste0("usethis::use_data(", ob, ", overwrite=TRUE)") )
      if (checking) {
        print(codetosource)
      } else {
    ...
    # make_vector_and_use_data(names_datasets)
    # make_vector_and_use_data(names_datasets, checking = FALSE)
