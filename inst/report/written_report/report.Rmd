...
The analysis used the [EJAM tool](https://ejanalysis.org/ejamdocs) 
`r params$app_version_header_text` with demographic data based on the Census
...
The `r EJAM:::global_or_param("app_title")` (EJAM) `r EJAM:::global_or_param("app_version_header_text")` was used to develop this analysis. This tool is a user-friendly web app that can summarize residential population and environmental conditions for any list of places in the nation.

### Spatial resolution of data

The analysis used the multisite tool `r EJAM:::global_or_param("app_version_header_text")`
with residential population data based on the Census Bureau's 
`r params$acs_version` American Community Survey (ACS) 5-year summary file, 
and the corresponding version of Decennial Census
information on blocks, with recent geographic boundaries and FIPS codes for 
blockgroups, tracts, counties, and States.

Demographic and environmental indicators were at blockgroup resolution (or tract level
for some indicators such as ambient air pollutant levels or air toxics indicators
as noted in technical documentation).
Locations of residential populations were based on Census block points 
(the highest resolution available from the Census Bureau) 
and then summarized by blockgroup, then by 
location analyzed, and finally overall across the locations as a whole.

See [EJAM package documentation](https://ejanalysis.org/ejamdocs) for methodology details.
...
-   EJAM `r EJAM:::global_or_param(".app_title")`
