...
ejam_report_tester <- function(sitepoints = testpoints_100, radius = 3) {
  ...
  old_envt_OPENSSL_CONF <- Sys.getenv("OPENSSL_CONF")
  Sys.setenv(OPENSSL_CONF = "/dev/null")
  on.exit(Sys.setenv(OPENSSL_CONF = old_envt_OPENSSL_CONF))

  old_option_scipen <- options("scipen")
  options(scipen = 999)
  on.exit(options(scipen = old_option_scipen))

  colcounter = EJAM:::colcounter

  x <- ejamit(sitepoints = sitepoints, radius = radius)
