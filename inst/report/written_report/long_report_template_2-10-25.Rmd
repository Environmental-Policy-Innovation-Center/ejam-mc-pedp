...
editor_options:
  markdown:
    wrap: 72
linenumbers: true
abstract: "Abstract - We analyzed baseline residential populations and environmental conditions in communities
  living `r params$where` `r params$facilities_studied`. The analysis used EJAM
  tool version `r params$app_version` with residential population data based
  on the Census Bureau’s `r params$acs_version` American Community Survey (ACS). The
  analysis found that x PLACEHOLDER."
params:
  testmode: TRUE
  ...
  fundingsource: NA
  acs_version: NA
  app_title: "`r EJAM:::global_or_param('app_title')`"
  app_version: "`r EJAM:::global_or_param('app_version')`"
  ejscreen_version: NA
...
if (is.na(params$acs_version)) {
  params$acs_version <- EJAM:::description_file$get("VersionACS")
}
if (is.na(params$ejscreen_version)) {
  # acs_version_global      <- EJAM:::description_file$get("VersionACS")
  # ejscreen_version_global <- EJAM:::description_file$get("VersionEJSCREEN")
  version <- EJAM:::description_file$get("VersionEJSCREEN")
...
An analysis was conducted to characterize baseline environmental
...
analyzed baseline residential population and environmental conditions in communities living
`r params$where`. 
or
  in communities living
`r params$within_x_miles_of` selected sites.

The analysis used the [`r params$app_title`](https://ejanalysis.org/ejamdocs) version
`r params$app_version` with demographic data based on the Census
...
The `r EJAM:::global_or_param("app_title")` was used to develop this analysis.
The [multisite tool](https://ejanalysis.org/ejamdocs)
...
indicators were at blockgroup resolution. Locations of
residential populations were at block resolution (the highest resolution
available from the Census Bureau) and then summarized by blockgroup. Once
...
See [EJSCREEN methodology](https://web.archive.org/web/20250118193121/https://www.epa.gov/ejscreen/technical-information-and-data-downloads)
for details.
...
below, and are described in detail in the EJSCREEN documentation, at
...
fraction of the parent blockgroup's Census 2020 population (which is
not quite the same as the ACS population count). That block weight is a
fraction of the parent blockgroup and is used to estimate how many of
the blockgroups residents are nearby or in the buffer.
...
how the distance to each Census block is measured. While EJSCREEN uses
ESRI's ArcGIS calculations, EJAM calculates the distance using
...
See [EJSCREEN methodology](https://web.archive.org/web/20250118193121/https://www.epa.gov/ejscreen/technical-information-and-data-downloads) for details.
...
geographic locations such as Census blockgroups
