...
Some functions in the package had incomplete unit test coverage at least as of 
mid 2025, so ideally more unit tests would be added in those cases.

You can see a grouped, organized list of unit test files *within the source
code* of a function **`test_ejam()`**, which is not exported but is loaded by the package.
...
**`EJAM/tests/test_coverage_check.R`**. The `test_ejam()` function noted
...
Also see the [covr package](https://covr.r-lib.org/) and the related RStudio addin, 
and see `devtools::test_coverage()` which is a shortcut for `covr::package_coverage()` plus `covr::report()`.
...
-   if the web app UI changes - then special tests of the web app functionality
    ...
    shinytest2](dev-run-shinytests.html). Also note that any screenshots for any
...
groups are passing, skipped, etc. To do this, note there is an unexported function called
**`test_ejam()`** that you can use to run the unit tests interactively.
...
of test file names in the source code of that function, `test_ejam()` or
you can just try to use it and it will stop and tell you which files are missing from
the list in the source code.

### Running tests of the web app functionality

There are recorded interactions used as tests of the web app, as explained in
[Testing EJAM App with shinytest2](dev-run-shinytests.html).
