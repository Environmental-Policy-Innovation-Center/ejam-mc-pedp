...
average resident in each blockgroup.
...
from blockgroup residential population data and environmental indicators and an estimate of
what fraction of each blockgroup is inside each site. For proximity analysis
...
that is what the radius would be if the block were circular, and it is
based on area = pi \* block_radius_miles\^2 or block_radius_miles = sqrt(area /
pi) where area is in square miles. However through EJAM v2.32.5, that field
...
These sites have no blocks within 1 mile, but do have blocks within 1.5 miles:
```{r}
