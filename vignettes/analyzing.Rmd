...
-   **`ejamapp()`** launches the web app locally (to run in RStudio on a single
    computer rather than on a server)

-   **`ejamit()`** provides most results in just one function (if you already
    have a list of places to analyze), as shown in the [Basics - Quick Start
Guide](basics.html)

# Tools for Exploring Results

For a standard analysis, the basic tools like `ejam2report()`, `ejam2excel()`,
`ejam2map()`, etc. let you explore results, as shown in the [Basics - Quick Start
Guide](basics.html).
...
# if (exists("frs") && exists("frs_by_naics")) {
  ...
  link1 <- url_echo_facility(regids, as_html = T)
...
#
# sapply(url_countyhealth(fips = cfips), browseURL)
